...
Amazon VPC (Virtual Private Cloud ) enables to have complete control over AWS virtual networking environment. Given this control, have you ever wondered how new Amazon VPC features might affect the way you design your AWS networking infrastructure, or even change existing architectures that you use today? Let's  explore the new design and capabilities of Amazon VPC and how you might use them.
...
so firstly let's take a step back in

time and we're going to rewind back to

around about November 2018
...
C and that sort of thing so we've got

multiple availability zones we've got

public subnets private subnets and then

you can deploy ec2 instances inside your

subnets in the availability zones in the

V PC in the region

and so I I always use ec2 is kind of

like the base level service here just to

use as an example for connectivity and

that sort of thing it just makes a lot
...
also the public Internet and we just

build a default route to the igw we have

an elastic IP assigned to our instance a

here and we can then communicate with

all of these public services and also

the public internet now for our private

subnet

actually send traffic via in that

instance instead of natively talking

directly to the public Internet and then
...
left-hand side here you can also talk to

s3 and dynamodb privately using VPC

endpoints and we'll use prefix lists

there to communicate privately and a VPC

endpoint is almost like a bridge between

say s3 and your V PC or dynamodb and

your B PC then we might want to connect

to our own premises so we'll use Direct

Connect or VPN maybe some direct connect

gateway we can hear our V PC with other

V pcs directly in a point-to-point

fashion either within a region or across

to another region you'll notice we build

a tower routes and left hand side here

again

transit gateway which was a service that

was launched in about November 2018 and

transit gateway allows you to connect to

many VPC so up to 5,000 V pcs can

communicate to each other over a transit

gateway and we can also use VPN from an

on-premises to a transit gateway
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTc1Njk4NTYxNF19
-->